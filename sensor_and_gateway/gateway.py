...
def handle_serial_data(s: serial.Serial) -> None:
    door_microbit_list = ["room", "location", "light_level", "sound_level",
                          "door_locking", "dist_str_in", "dist_str_out", "people_count"]
    window_microbit_list = ["room", "location", "light_level", "movement"]
    output_dict = {}
    output_str = ""
    ...
    if (len(payload_list) == 8):
        # If the payload is from door
        for i in range(0, len(door_microbit_list)):
            output_dict[door_microbit_list[i]] = payload_list[i]
        ...
        for i in range(0, len(window_microbit_list)):
            output_dict[window_microbit_list[i]] = payload_list[i]
